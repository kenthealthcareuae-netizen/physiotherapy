...
const UnifiedContactWidget = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'يرجى إدخال اسمك الكامل';
    }
...
Timestamp: ${new Date().toLocaleString('en-US', { timeZone: 'Asia/Dubai' })}`,
  ...
  const handleFormSubmit = async () => {
    ...
    if (success) {
      toast({
        ...
        description: 'شكراً لك! سنتواصل معك قريباً لتأكيد جلستك المجانية.',
        variant: 'default'
      });
      setIsOpen(false);
  ...
  const handleWhatsAppSubmit = async () => {
    ...
    if (success) {
      const message = `مرحباً، اسمي ${formData.name.trim()} ورقم هاتفي ${formData.phone.trim()}. أود الاستفسار عن علاج العلاج الطبيعي.`;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span className="hidden sm:inline" dir="rtl">احجز جلستك المجانية</span>
          <span className="sm:hidden" dir="rtl">احجز مجاناً</span>
        </Button>
      </motion.div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div
              className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl fixed bottom-4 left-4"
              initial={{ y: '100%', x: '-100%' }}
              animate={{ y: 0, x: 0 }}
              exit={{ y: '100%', x: '-100%' }}
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-gray-800" dir="rtl">
                  احجز جلستك المجانية
                </h3>
                <Button
                  variant="ghost"
              ...
              <form className="space-y-4">
                <div>
                  <Label htmlFor="name" className="text-sm font-medium text-gray-700" dir="rtl">
                    الاسم الكامل *
                  </Label>
                  <Input
                    id="name"
                  ...
                  />
                  {errors.name && (
                    <p className="text-red-500 text-xs mt-1" dir="rtl">{errors.name}</p>
                  )}
                </div>

                <div>
                  <Label htmlFor="phone" className="text-sm font-medium text-gray-700" dir="rtl">
                    رقم الهاتف *
                  </Label>
                  <Input
                    id="phone"
                  ...
                  />
                  {errors.phone && (
                    <p className="text-red-500 text-xs mt-1" dir="rtl">{errors.phone}</p>
                  )}
                </div>
                ...
                <div className="space-y-3 pt-4">
                  ...
                  >
                    ...
                    {isSubmitting ? 'جاري الإرسال...' : 'واتساب'}
                  </Button>
                ...
                <div className="bg-blue-50 p-3 rounded-lg mt-4">
                  <p className="text-xs text-blue-800" dir="rtl">
                    <strong>ملاحظة:</strong> نحن نحترم خصوصيتك؛ معلوماتك سرية ومحمية.
                  </p>
                </div>
              </form>
            </motion.div>
